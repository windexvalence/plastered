...
async def test_submit_album_search_form_endpoint(client: TestClient) -> None:
    ...
    with patch(
        "plastered.api.server.manual_search_action", return_value={"fake": "data"}
...
@pytest.mark.parametrize(
    "rec_type, expected_rt_cli_override_arg", [
        ("track", ["track"]), ("album", ["album"]), ("all", ["album", "track"]),
    ]
)
def test_scrape_endpoint(
    ...
    with patch("plastered.api.server.scrape_action", return_value=None) as mock_scrape_action, patch(
        "plastered.api.server.get_app_settings", return_value=valid_app_settings
        ...
        mock_get_app_settings.assert_called_once_with(
            Path(valid_config_filepath), cli_overrides={"SNATCH_ENABLED": snatch, "REC_TYPES": expected_rt_cli_override_arg}
        )
