...
- `make test` — run all tests (non-containerized). Sets `PYTHONPATH`/`APP_DIR` and calls `src/python/tests/tests_entrypoint.sh`.
...
- `src/python/tests/conftest.py` sets `PLASTERED_CONFIG` to `examples/config.yaml` before any imports — config loads eagerly at import time, so import order matters.

## Architecture

### Request → snatch flow (CLI `scrape`)

1. `cli.py` builds an `AppSettings` via `get_app_settings()`, merging `config.yaml` + env vars (`PLASTERED_*`) + CLI overrides, then calls `scrape_action` (`src/python/plastered/actions/common_actions.py`).
2. `LFMRecsScraper` (`src/python/plastered/scraper/lfm_scraper.py`) uses Playwright (rebrowser-playwright) to scrape recommendation pages into `dict[EntityType, list[LFMRec]]`.
3. `ReleaseSearcher` (`src/python/plastered/release_search/release_searcher.py`) is the orchestrator. It owns four httpx API clients (LFM, MusicBrainz, RED, RED-snatch), a `RunCache`, and a `SearchState`. It wraps each `LFMRec` in a `SearchItem` and runs them through the processor chain, then snatches matches.
...
### Config (`src/python/plastered/config/app_settings.py`)
...
### API clients (`src/python/plastered/utils/httpx_utils/`)

All clients subclass `ThrottledAPIBaseClient` (`base_client.py`), which wraps `httpx.Client` with a custom retry transport (`HTTPXRetryTransport`, tenacity-based) and per-API rate limiting. Each client takes a shared `RunCache`. Responses are optionally cached on disk via diskcache (`src/python/plastered/run_cache/run_cache.py`); cache types are `API` and `SCRAPER`, managed by the CLI `cache` command.

### Web server (`src/python/plastered/api/`)
...
### Persistence (`src/python/plastered/db/`)
...
- Raising bare `Exception` is banned (`TRY002`); use the typed exceptions in `src/python/plastered/utils/exceptions.py`.
- Domain models are re-exported from `src/python/plastered/models/__init__.py` and clients from `src/python/plastered/utils/httpx_utils/__init__.py` — import from the package, not the submodule.
