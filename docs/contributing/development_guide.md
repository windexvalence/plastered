# Development Guide

## Pre-requisites
1. Install [`uv`](https://docs.astral.sh/uv/) if you do not already have it

## Setup / Installation

1. Clone the repo from `git@github.com:windexvalence/plastered.git`
2. Explore the full list of local development options by running `make` to see the help output.
3. Install [`prek`](https://prek.j178.dev/installation/) for pre-commit hook support
4. From the repo root directory, run `prek install`

### Optional: Code Editor Setup

Once `uv` is installed, you can follow this one-time setup for creating a host virtualenv. Make sure to run all commands from the root `plastered` repo directory:

1. Install the correct Python version:
    ```shell
    uv python install 3.14
    ```
2. Create a dedicated virtualenv:
    ```shell
    uv venv
    ```
3. Install the dependencies in the venv you just created in step 2:
    ```shell
    uv sync --all-groups
    ```
4. Optionally, configure your code editor / IDE of choice to use the uv-managed virtualenv located in the `.venv` directory at the root `plastered` project directory.

## Testing

1. To run code formatting checks, run: `make fmt-check`. 
    
    * If this command raises formatting errors, you will need to run the code auto-formatter by running: `make fmt`.

    * If this command and/or the `make fmt` command raises errors, you will need to manually address those and re-run the `make fmt` command to verify if the raised errors have been addressed.

2. To run ALL unit tests, run: `make test`

    * To run only a specific test file, run the make command with `TEST_TARGET` set to the relative test file's path. For example, the following will only run tests defined in `test_http_utils.py`:
        ```shell
        make test TEST_TARGET=tests/utils_tests/test_http_utils.py
        ```
    
    * To run only a specific test function within a specific test file, run the make command with `TEST_TARGET` set to the relative test file's path followed by `::<target-test-function-name-here>`. For example, the following will only run the `test_throttle` test function in `test_http_utils.py`:
        ```shell
        make test TEST_TARGET=tests/utils_tests/test_http_utils.py::test_throttle
        ``` 


### Testing the Server

Run the following:

```shell
make docker-server APP_CONFIG_DIR=<path to host plastered config directory>
```

Then open http://localhost:8000 in your browser.

## Other testing commands

Run `make` to list the other available targets and their descriptions.

## Cutting a release

Versioning and release notes are driven by [conventional commits](https://www.conventionalcommits.org/) via [cocogitto](https://docs.cocogitto.io/) (`cog`, configured in `cog.toml`): PRs are squash-merged, so each PR title (enforced by the `PR title` workflow) becomes a conventional commit on `main`, and the commits since the latest `vX.Y.Z` tag determine the next version and the generated release notes. The package version itself is derived from the git tag by [setuptools-scm](https://setuptools-scm.readthedocs.io/en/latest/) (see `pyproject.toml`), so there is no version to bump by hand anywhere — no draft release, no hand-written notes, no `pyproject.toml` edit.

1. Run the `Release` workflow (Actions -> Release -> Run workflow, from `main`). It checks the commits since the latest tag are conventional and warrant a bump, computes the pending version, runs the full check/test suite on both release architectures, and build-validates the app image without publishing anything.
2. Approve the `release` environment prompt. `cog bump --auto` tags the validated commit `vX.Y.Z` (tag-only — nothing is pushed to `main`), the GitHub release is created with the cog-generated changelog as its notes, and the `Publish` workflow is dispatched, which builds the per-arch images from the tag (stamping the image version from the tag via the `PLASTERED_RELEASE_TAG` build-arg) and pushes the multi-arch manifest (`:X.Y.Z` + `:latest`) to GHCR.
