...
from lastfm_recs_scraper.utils.exceptions import RedClientSnatchException, StatsTableException
...
class StatsTable():
    ...
    def __init__(
        ...
        if len(cell_idxs_to_style_fns) > self._num_cols or any([idx < 0 or self._num_cols <= idx for idx in cell_idxs_to_style_fns.keys()]):
            raise StatsTableException(f"Invalid cell_idxs_to_style_fns value. Must not contain more entries than table has columns.")
        self._per_row_cell_style_fns = {i: cell_idxs_to_style_fns[i] if i in cell_idxs_to_style_fns else None for i in range(self._num_cols)}
    ...
    def add_row(self, row: List[str]) -> None:
        stylized_row = [
            cell_data if not self._per_row_cell_style_fns[i] 
    ...
    def to_tsv_file(self) -> None:
        ...
        try:
            ...
            _LOGGER.error(f"Failed to write TSV file for {self.__class__.__name__} to filepath: {self._tsv_output_path}", exc_info=True)
...
class SkippedSummaryTable(StatsTable):
    ...
    def stylize_skip_reason_entry(reason: str) -> str:
        ...
        if entry_enum == SkippedReason.ABOVE_MAX_SIZE:
            return "dark_orange3"
        elif entry_enum == SkippedReason.ALREADY_SNATCHED:
            return "green"
        elif entry_enum == SkippedReason.NO_MATCH_FOUND:
            return "red3"
        else:
            return "white"
...
class FailedSnatchSummaryTable(StatsTable):
    ...
    def stylize_failure_reason_entry(failure_reason: str) -> str:
        ...
        if failure_enum == SnatchFailureReason.RED_API_REQUEST_ERROR:
            return "red3"
        elif failure_enum == SnatchFailureReason.FILE_ERROR:
            return "dark_orange3"
        else:
            return "bright_red"
...
class RunCacheSummaryTable(StatsTable):
    ...
    def stylize_cache_hit_rate_entry(hit_rate_str: str) -> str:
        ...
        if 0 <= hit_rate < 0.25:
            return "red3"
        elif 0.25 <= hit_rate < 0.5:
            return "dark_orange3"
        elif 0.5 <= hit_rate < 0.75:
            return "green_yellow"
        else:
            return "green"

    def __init__(self, cache_type_str: str, disk_usage_mb: str, hits: str, misses: str, hit_rate: str, directory_path: str):
        super().__init__(
...
def print_and_save_all_searcher_stats(
    ...
    SkippedSummaryTable(
        rows=skipped_rows, tsv_output_path=f"{output_filepath_prefix}_skipped.tsv"
    ).print_and_save()
