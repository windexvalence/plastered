...
from lastfm_recs_scraper.utils.constants import API_CACHE_DIRNAME, CACHE_DIRNAME, CACHE_TYPE_API, CACHE_TYPE_SCRAPER, SCRAPER_CACHE_DIRNAME
...
class RunCache:
    ...
    def __init__(self, app_config: AppConfig, cache_type: CacheType):
        ...
        if self._enabled:
            ...
            self._cache.expire()
            _LOGGER.info(f"Any newly added {self._cache_type.value} cache entries will expire on {self._expiration_datetime.strftime('%Y_%m_%d %H:%M:%S')}")
    ...
    def print_summary_info(self) -> None:
        ...
        print(f"    Cache hit rate (prior run): {float(hits) / float(hits + misses)}")
        print(f"----------------------------------")
    ...
    def load_data_if_valid(self, cache_key: Any, data_validator_fn: Callable) -> Any:
        ...
        try:
            ...
            _LOGGER.error(f"Encountered uncaught error during validation of {self._cache_type} data under cache key '{cache_key}'.")
