...
if TYPE_CHECKING:
    ...
    from plastered.release_search.processors.bases import AsyncFilterFuncs, FilterFuncs
...
class BaseFilter(SearchItemFilter):
    ...
    @classmethod
    async def a_process(
        cls, si: SearchItem, state: SearchState, **kwargs: Any
    ) -> SearchItem | None:  # pragma: no cover
        for a_func in cls.a_funcs:
            skip_reason = await a_func(si, state)
            if skip_reason:
                await cls._async_add_skipped_snatch(si=si, skip_reason=skip_reason)
                return None
        return si
    ...
    @classmethod
    async def _async_add_skipped_snatch(cls, si: SearchItem, skip_reason: SkipReason) -> None:
        """Adds a Skipped db record for the given `SearchItem` and `SkipReason`."""
...
class PreMBIDResolutionFilter(BaseFilter):
    ...
    a_funcs: ClassVar[AsyncFilterFuncs] = tuple([])
